...
def artist_url(band_name,page_num):
    if (page_num):
        ...
        return base_url + '/tabs/' + band_name.strip().replace(' ', '_') + '_tabs.htm'


#
# status,response = http.request(artist_url('All That Remains'))
# print response
tab_file = open('tab_links.txt','w')

tab_links = []
# html_source = open('all_that_remains_tabs.htm','r').read()
